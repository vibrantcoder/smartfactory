...
<div class="mb-5 flex flex-wrap items-center justify-between gap-3">
    <div>
        <p class="text-xs text-gray-500 mt-0.5">Log and track machine stoppages · impacts OEE Availability</p>
    </div>
    <div class="flex items-center gap-2">
        <template x-if="factories.length > 0">
            <select @change="currentFactoryId = $event.target.value; loadData()"
                        ...
                        <td class="px-4 py-3">
                            ...
                            <template x-if="row.category">
                                <span class="ml-1 text-xs rounded-full px-2 py-0.5"
                                      :class="row.category === 'planned' ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700'"
            ...
            <div>
                ...
                <select x-model="form.downtime_reason_id"
                    ...
                    <template x-for="r in reasons" :key="r.id">
                        <option :value="r.id" x-text="r.code + ' — ' + r.description + (r.category ? ' (' + r.category + ')' : '')"></option>
                    </template>
                </select>
            </div>
...
function downtimeManager(apiToken, factoryId, machines, reasons, factories) {
    return {
        ...
        async loadData() {
            ...
            try {
                const res = await fetch(`/api/v1/downtimes?${params}`, {
        ...
        computeKpi() {
            ...
            const reasonCounts = {};
            this.rows.forEach(r => {
                if (r.downtime_reason_id) {
                    reasonCounts[r.downtime_reason_id] = (reasonCounts[r.downtime_reason_id] || 0) + 1;
                }
            });
            const topId = Object.entries(reasonCounts).sort((a,b) => b[1]-a[1])[0]?.[0];
        ...
        reasonName(id)   { return id ? (this.reasons.find(r => r.id == id)?.description || '') : ''; },
        ...
        async closeDowntime(row) {
            ...
            await fetch(`/api/v1/downtimes/${row.id}`, {
                method:  'PUT',
                headers: { 'Authorization': `Bearer ${this.apiToken}`, 'Content-Type': 'application/json', 'Accept': 'application/json' },
